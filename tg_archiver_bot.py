...
def build_channel_progress(info: dict) -> str:
    ...
    lines = [
        "归档中...",
        f"消息组：{info.get('scanned', 0)}/{info.get('target_messages', '?') or 'full'}",
        f"已扫描消息组：{info.get('scanned', 0)}",
...
async def main() -> None:
    ...
    async def run_channel_task(status_message, link: str, limit: int | None, output_dir: Path, label: str, offset: int = 0, range_mode: bool = False) -> None:
        ...
        try:
            ...
            await status_message.edit(
                ...
                f"消息组：{summary.scanned_messages}/{summary.target_messages or 'full'}\n"
    ...
    async def handle_archive_channel(event):
        ...
        if CFG["silent_download_mode"] or not CFG["download_dir_aliases"]:
            target_dir = resolve_output_dir(None)
            target_dir.mkdir(parents=True, exist_ok=True)
            status = await event.reply(f"开始归档频道，准备扫描最新 {limit} 个消息组。\n输出目录：{target_dir}")
    ...
    async def handle_channel_choice(event):
        ...
        if choice == "custom":
            # 自定义输入：支持两种格式。
            #   单数字 N：下载最新 N 个消息组（从新到旧）。
            #   区间 A B：从旧到新，1=频道第一个消息组（最旧），下载第 A~B 个消息组。
            try:
            ...
            if isinstance(range_total, int) and range_total > 0:
                ...
                hint = (
                    f"📊 该频道共 {total_str} 个消息组。\n\n"
                    "请发送数字，两种格式：\n\n"
                    "📌 区间（两个数字，从旧到新）：\n"
                    f"  序号 1 = 第一个消息组（最旧），{total_str} = 最新一个\n"
                    f"  例如：1 5   → 下载第 1~5 个消息组\n"
                    f"  例如：50 150 → 下载第 50~150 个消息组\n"
                    f"  最大序号 {total_str}\n\n"
                    "📌 单数字（最新 N 个消息组）：\n"
                    "  例如：7   → 下载最新 7 个消息组"
                )
        ...
        if CFG["silent_download_mode"] or not CFG["download_dir_aliases"]:
            await event.edit(
                f"开始归档频道，准备扫描{'全部可用' if limit is None else f'最新 {limit} 个消息组'}。"
            )
    ...
    async def handle_fallback(event):
        ...
        # 拦截「频道自定义」的文本输入。两种格式：
        #   单数字 N：最新 N 个消息组（从新到旧，range_mode=False）。
        #   区间 A B：从旧到新，1=频道第一个消息组（最旧），第 A~B 个消息组（range_mode=True）。
        ...
        if range_pending is not None:
            ...
            if len(parts) == 1:
                ...
                range_label = f"{range_link} (最新 {single_n} 个消息组)"
                if CFG["silent_download_mode"] or not CFG["download_dir_aliases"]:
                    target_dir = resolve_output_dir(None)
                    target_dir.mkdir(parents=True, exist_ok=True)
                    status = await event.reply(f"开始归档频道，最新 {single_n} 个消息组。\n输出目录：{target_dir}")
            ...
            if len(parts) == 2:
                ...
                if isinstance(range_total, int) and range_total > 0 and end_seq > range_total:
                    await event.reply(f"结束序号 {end_seq} 超过频道总消息组数 {range_total}，请重新输入。")
                    return
                # 序号 -> offset/limit：从旧到新迭代（reverse=True），offset=start_seq-1
                # 跳过最旧的前若干个消息组，limit=end_seq-start_seq+1 取该区间。
                ...
                if CFG["silent_download_mode"] or not CFG["download_dir_aliases"]:
                    target_dir = resolve_output_dir(None)
                    target_dir.mkdir(parents=True, exist_ok=True)
                    status = await event.reply(f"开始归档频道，序号 {start_seq}-{end_seq}（共 {range_limit} 个消息组，从旧到新）。\n输出目录：{target_dir}")
